...
export async function GET(request: NextRequest) {
  try {
    ...
    let sql = `
      ...
      FROM events e LEFT JOIN agents a ON e.agent_id = a.id LEFT JOIN tasks t ON e.task_id = t.id WHERE 1=1
    `;
...
export async function POST(request: NextRequest) {
  try {
    ...
    await run(
      ...
      [id, body.type, body.agent_id || null, body.task_id || null, body.message, body.metadata ? JSON.stringify(body.metadata) : null]
    );

    return NextResponse.json({ id, type: body.type, message: body.message }, { status: 201 });
  } catch (error) {
